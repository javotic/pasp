...
export class GeneralesService {
  ...
  consultaCatalogoTituloCarrera() {
    const url = `${environment.BASEENPOINT_BUS_SEI}/servidorpublico/CONSULTACATTITULOCARRERA`;

    const formData = new FormData();
    formData.append('IDTICARRERA', '');

    return this.http.post<RespuestaApi<ConsultaCatTituloCarrera>>(`${url}`, formData)
      .pipe(
        catchError(e => {
          if (e.status === 400) {
            return throwError(e);
  ...
  consultaCatalogoTipoDiploma() {
    ...
    const url = environment.BASEENPOINT_BUS_SEI + '/catalogos/CONSULTACATTIPOCERTIFICADO';
    return this.http.post<RespuestaApi<ConsultaCatTipoCertificado>>(`${url}`, null)
      .pipe(
        catchError(e => {
          if (e.status == 400) {
            return throwError(e);
  ...
  consultaCatalogoColonias(idEstado: string, idMunicipio: string) {
    const formData = new FormData();
    formData.append('IDPAIS', 'MEX');
    formData.append('IDPESTADO', idEstado);
    formData.append('IDMUNICIPIO', idMunicipio);
    formData.append('IDCOLONIA', '');

    const url = environment.BASEENPOINT_BUS_SEI + '/catalogos/CONSULTACATCOLONIAS';

    return this.http.post<RespuestaApi<ConsultaCatColonia>>(`${url}`, formData)
      .pipe(
        catchError(e => {
          if (e.status == 400) {
            return throwError(e);
